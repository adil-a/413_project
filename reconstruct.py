...
def map_filename(data_source, npz):
...
if __name__ == "__main__":
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        data_sdfs_futures = [executor.submit(map_filename, args.data_source, npz) for npz in npz_filenames]
        data_sdfs = [data_sdf.result() for data_sdf in data_sdfs_futures]
    
    all_data = {}
    for ii, (npz, data_sdf) in enumerate(data_sdfs):
        all_data[npz] = data_sdf
    ...
    for ii, npz in enumerate(npz_filenames):
        ...
        data_sdf = all_data[full_filename]
        ...
        for k in range(repeat):
            ...
            data_sdf[0] = data_sdf[0].to(device)
            data_sdf[1] = data_sdf[1].to(device)
